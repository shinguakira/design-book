...
export default function CheckboxShowcase() {
  ...
  const set = (k: keyof typeof v, val: boolean) => setV((s) => ({ ...s, [k]: val }));
...
function CustomCheck() {
  ...
  return (
    <button onClick={() => setOn((v) => !v)} className="flex items-center gap-2 group">
      <span
      ...
      >
        <span
          className={`w-5 h-5 rounded-md flex items-center justify-center ${
            all || indet ? "bg-zinc-900 border-2 border-zinc-900" : "border-2 border-zinc-300"
          }`}
        >
        ...
        >
          <span
            className={`w-4 h-4 rounded flex items-center justify-center ${
              items[k] ? "bg-zinc-900 border-2 border-zinc-900" : "border-2 border-zinc-300"
            }`}
          >
...
function DescCheck() {
  const [on, setOn] = useState(true);
  return (
    <button onClick={() => setOn((v) => !v)} className="flex items-start gap-3 text-left max-w-md">
      <span
        className={`mt-0.5 w-5 h-5 rounded-md flex items-center justify-center shrink-0 ${
          on ? "bg-blue-600 border-2 border-blue-600" : "border-2 border-zinc-300"
        }`}
      >
        {on && <Check className="w-3.5 h-3.5 text-white" />}
      </span>
        ...
        return (
          <label
            ...
            className={`relative cursor-pointer rounded-lg border-2 p-3 transition ${
              sel ? "border-zinc-900 bg-zinc-50" : "border-zinc-200 hover:border-zinc-300"
            }`}
            ...
            <button key={k} onClick={() => toggle(setHearts, k)}>
              <Heart
                className={`w-6 h-6 transition ${
                  on ? "fill-rose-500 text-rose-500" : "text-zinc-300 hover:text-rose-300"
                }`}
              />
            </button>
          );
        ...
        {["a", "b", "c", "d", "e"].map((k) => {
          ...
          return (
            <button key={k} onClick={() => toggle(setStars, k)}>
              <Star
                className={`w-6 h-6 transition ${
                  on ? "fill-amber-400 text-amber-500" : "text-zinc-300 hover:text-amber-300"
                }`}
              />
            </button>
          );
          ...
          >
            <span
              className={`w-5 h-5 rounded-md flex items-center justify-center border-2 transition ${
                sel ? COLORS[o.color].box : `border-zinc-300 ${COLORS[o.color].ring}`
              }`}
            >
