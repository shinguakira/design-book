...
import { ChevronDown, ChevronUp, GripVertical, Trophy, Flame } from "lucide-react";
...
function Reorderable() {
  ...
  return (
    <ul className="rounded-md border border-zinc-200 bg-white divide-y divide-zinc-100">
      {items.map((f, i) => (
        <li key={f.name} className="flex items-center gap-3 px-3 py-2 hover:bg-zinc-50 group">
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <p className="text-sm text-zinc-700 leading-relaxed">
        「番号 + 名前」の1行リスト。 ランキング・目次・タスク・上位リスト等で頻出。
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Frame label="Plain serif (newspaper)" note="紙面・目次・PubMed風">
          <ol className="divide-y divide-zinc-200">
            {FOODS.map((f, i) => (
              <li key={f.name} className="grid grid-cols-[28px_1fr] gap-3 py-1.5 items-baseline">
        ...
        <Frame label="Big faded background" note="番号を大きく薄く、視覚的アクセント">
          <ul className="space-y-1">
            {FOODS.map((f, i) => (
              <li key={f.name} className="relative flex items-center gap-4 px-2 py-1.5">
                <span
        ...
        <Frame label="With right meta" note="数字+名前+右端に値 (スコア・件数)">
          <ul className="divide-y divide-zinc-200">
            {FOODS.map((f, i) => (
              <li key={f.name} className="grid grid-cols-[28px_1fr_auto] gap-3 py-2 items-baseline">
                <div className="text-right text-sm font-semibold text-zinc-400 tabular-nums">
                  {i + 1}
                </div>
                <div className="text-sm">{f.name}</div>
                <div className="text-sm font-mono tabular-nums text-zinc-600">{f.score}</div>
              </li>
            ))}
          </ul>
        </Frame>
        ...
        <Frame label="Bordered card (tiered)" note="行ごとに小カード、左辺がランク帯">
          <ul className="space-y-1.5">
            {FOODS.map((f, i) => {
              const colors =
                i < 3 ? "border-l-amber-500" : i < 6 ? "border-l-zinc-400" : "border-l-orange-700";
              return (
                <li
                  key={f.name}
        ...
        <Frame label="Bar overlay" note="番号+名前の背景がスコア比率の塗りつぶしバーになる">
          <ul className="space-y-1">
            {FOODS.map((f, i) => {
              const max = FOODS[0].score;
              const pct = (f.score / max) * 100;
              return (
                <li key={f.name} className="relative h-8 rounded-md overflow-hidden bg-zinc-50">
                  <div
                  ...
                  <div className="relative h-full flex items-center gap-3 px-3 text-sm">
                    <span className="font-semibold tabular-nums text-zinc-700 w-5">{i + 1}</span>
                    <span className="flex-1">{f.name}</span>
                    <span className="font-mono text-xs text-zinc-600 tabular-nums">{f.score}</span>
                  </div>
                </li>
              );
            })}
        ...
        <Frame label="Tier blocks (S/A/B/C)" note="ティアリスト・カードゲーム評価風">
          <ul className="space-y-1">
            {FOODS.map((f, i) => {
              ...
              return (
                <li key={f.name} className="flex items-center gap-3 rounded-md overflow-hidden">
                  <div
                  ...
                  <div className="text-xs font-mono text-zinc-400 tabular-nums pr-2">#{i + 1}</div>
                </li>
              );
            })}
          </ul>
        ...
        <Frame label="Subway line" note="ロードマップ・タイムライン。縦線で繋ぐ">
          <ul className="relative pl-3">
            ...
            {FOODS.map((f, i) => (
              <li key={f.name} className="relative flex items-center gap-4 py-2">
              ...
              {FOODS.map((f, i) => (
                <li key={f.name} className="flex items-center gap-3 text-sm">
                  <div
                    ...
                    style={{
                      color: "#22d3ee",
                      textShadow: "0 0 6px #22d3ee, 0 0 12px #06b6d4, 0 0 24px rgba(6,182,212,0.4)",
                    }}
                  >
                    {String(i + 1).padStart(2, "0")}
                  </div>
        ...
        {/* 18 — Dotted leader */}
        <Frame label="Dotted leader" note="メニュー・目次の古典タイポ。番号 ··· 名前 ··· 値">
        ...
        <Frame label="Receipt / ticket" note="モノ等幅・破線・KASA音">
          <div className="bg-[#fbf6e7] border-2 border-dashed border-zinc-400 rounded-sm p-4 font-mono">
            ...
            <div className="border-t border-dashed border-zinc-400 my-2" />
            <div className="text-center text-[10px] text-zinc-500">ありがとうございました</div>
          </div>
        </Frame>

        {/* 20 — Card grid (trading cards) */}
        <Frame label="Trading card grid" note="リスト→グリッド化。カードゲーム / コレクション" span>
          <ul className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {FOODS.map((f, i) => (
              ...
              >
                ...
                <div className="text-[11px] font-mono tabular-nums opacity-90">★ {f.score}</div>
              </li>
            ))}
          </ul>
        </Frame>
      ...
      <section className="rounded-lg border border-blue-200 bg-blue-50 p-5 text-sm text-blue-900">
        ...
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <b>視線誘導が必要</b> (ランキング) → 番号を強く: Medal / Big faded / Gradient bubble /
            Neon
          </li>
          <li>
            <b>情報を素早く読ませたい</b> (目次) → 番号は控えめに: Plain serif / Boxed / Striped /
            Dotted leader
          </li>
          <li>
          ...
          <li>
            <b>世界観を出す</b> → Roman (書籍) / Hexagon (ゲーム) / Neon (配信) / Receipt (アナログ)
            / Trading card (コレクション)
          </li>
          <li>
