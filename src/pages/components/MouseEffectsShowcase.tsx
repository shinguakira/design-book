...
function ParticleTrail() {
  const [parts, setParts] = useState<{ id: number; x: number; y: number; hue: number }[]>([]);
...
function ClickRipple() {
  ...
  return (
    ...
    >
      ...
      `}</style>
      <div className="text-white font-medium pointer-events-none">クリックして波紋を出す</div>
      {ripples.map((r) => (
        <div
          key={r.id}
...
function SparkleTrail() {
  ...
  return (
    ...
    >
      ...
      `}</style>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-pink-400 text-sm font-medium">動かしてキラキラ ✦</div>
      </div>
      {stars.map((s) => (
        <div
          key={s.id}
...
function TiltCard() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="text-[10px] uppercase tracking-widest opacity-80">3D TILT</div>
    ...
    <div className="h-64 rounded-lg bg-zinc-50 flex items-center justify-center">
      <div className="flex flex-col gap-3 text-zinc-900 text-lg">
        {["Home", "Projects", "About", "Contact"].map((t) => (
          <button key={t} className="relative group inline-flex items-center px-1 py-0.5">
            ...
            <span
              className="absolute -inset-2 rounded-md opacity-0 group-hover:opacity-100 transition pointer-events-none"
              style={{
                background:
      ...
      {trail.map((t, i) => (
        <div
          ...
          style={{
            ...
            height: 20 - i * 1.8,
            transform: "translate(-50%, -50%)",
            background: "radial-gradient(circle, #67e8f9 0%, #22d3ee 60%, transparent 100%)",
            opacity: 1 - i * 0.1,
          }}
        />
      ))}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #f43f5e 0%, #f59e0b 40%, #10b981 70%, #06b6d4 100%)",
          clipPath: active ? `circle(80px at ${pos.x}% ${pos.y}%)` : "circle(0px at 50% 50%)",
        }}
      />
  ...
  const [bursts, setBursts] = useState<
    {
      ...
      x: number;
      y: number;
      pieces: { dx: number; dy: number; c: string; r: number }[];
    }[]
  >([]);
  const counter = useRef(0);
  ...
  return (
    <div className="max-w-5xl space-y-3">
      <p className="text-sm text-zinc-700 leading-relaxed">
        マウスエフェクト16種。光の粒トレイル / カスタムカーソル / スポットライト / 波紋 / キラキラ /
        3D傾き / 磁石ボタン / ホバー光 / 遅延紐 / インクトレイル / パララックス / テキスト逃避 /
        画像リビール / 紙吹雪 / Velocity / 文字バラバラ。
      </p>
      ...
      <section className="rounded-lg border border-blue-200 bg-blue-50 p-5 text-sm text-blue-900">
        ...
        <ul className="list-disc pl-5 space-y-1">
          <li>
            常時パーティクル生成は CPU/バッテリーを食う → スロットリング必須 (このページは 20-60ms
            間隔)
          </li>
