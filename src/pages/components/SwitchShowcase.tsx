...
export default function SwitchShowcase() {
  ...
  const set = (k: keyof typeof vals, v: boolean) =>
    setVals((s) => ({ ...s, [k]: v }))
    ...
    <div className="max-w-5xl space-y-3">
        ...
        {[
          ...
          <div key={v.id} className="flex items-center gap-2">
            <Sw
              ...
              onChange={(val) =>
                set(['a', 'b', 'c', 'd', 'e'][i] as keyof typeof vals, val)
              }
              className={
                [vals.a, vals.b, vals.c, vals.d, vals.e][i]
                  ? v.on
                  : 'bg-zinc-300'
              }
              knob="bg-white"
      ...
      <Frame label="6. Icons inside" note="夜/昼 のトグル">
        ...
        >
          ...
          >
            {vals.e ? <Moon className="w-3 h-3 text-indigo-900" /> : <Sun className="w-3 h-3 text-amber-500" />}
          </span>
        </button>
      </Frame>
