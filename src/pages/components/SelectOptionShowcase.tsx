...
function S1() {
  ...
  return (
    <div ref={ref} className="relative">
      <Trigger onClick={() => setOpen(!open)} open={open}>{v}</Trigger>
      {open && (
        <Panel>
...
function S7() {
  ...
  return (
    <div ref={ref} className="relative">
      <Trigger onClick={() => setOpen(!open)} open={open} w={240}>{v}</Trigger>
      {open && (
        <Panel w={280}>
          {items.map((it) => (
            <button
...
function S8() {
  const items = [
    { t: 'Q4 Product Launch', meta: '25 tasks · 3 members', time: '2h ago', tag: 'On track', ok: true },
    { t: 'Design System v2', meta: '11 tasks · 2 members', time: '1d ago', tag: 'At risk', ok: false },
          ...
          {items.map((it) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium">{it.t}</div>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${it.ok ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                  {it.tag}
                </span>
              </div>
          ...
          {items.map((it) => (
            ...
            >
              ...
              <div className="mt-1 h-1 bg-zinc-200 rounded-full overflow-hidden">
                <div className={`h-full ${it.u > 80 ? 'bg-rose-500' : it.u > 50 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${it.u}%` }} />
              </div>
            </button>
          ))}
        </Panel>
...
function S11() {
  ...
  const label = sel.size === 0 ? '選択なし' : sel.size === 1 ? Array.from(sel)[0] : `${sel.size}件選択中`
  return (
    <div ref={ref} className="relative">
      <Trigger onClick={() => setOpen(!open)} open={open}>{label}</Trigger>
      {open && (
        <Panel>
          {items.map((o) => (
            <button
              ...
              className="w-full px-3 py-1.5 text-sm flex items-center gap-2 hover:bg-zinc-100"
            >
              <span className={`w-4 h-4 rounded border-2 grid place-items-center flex-none ${sel.has(o) ? 'bg-zinc-900 border-zinc-900' : 'border-zinc-300'}`}>
          ...
          {items.map((o) => (
            ...
            >
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100">
                <button className="p-1 rounded hover:bg-zinc-200"><Star className="w-3 h-3 text-zinc-600" /></button>
                <button className="p-1 rounded hover:bg-zinc-200"><Trash2 className="w-3 h-3 text-zinc-600" /></button>
              </div>
            </div>
          ))}
        </Panel>
            ...
            ? [80, 60, 90, 50].map((w, i) => (
                <div key={i} className="px-3 py-2 flex items-center gap-2">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="h-2 rounded bg-zinc-200 animate-pulse" style={{ width: `${w}%` }} />
            ...
            >
              {o.split(new RegExp(`(${q})`, 'i')).map((p, k) =>
                q && p.toLowerCase() === q.toLowerCase() ? (
                  <mark key={k} className="bg-amber-200 text-zinc-900 font-semibold rounded">{p}</mark>
                ) : (
                  <span key={k}>{p}</span>
                ),
              )}
...
export default function SelectOptionShowcase() {
  const patterns: { title: string; slots: string; Comp: () => React.ReactNode }[] = [
    ...
    { title: 'Rich card (badges + meta row)', slots: '[ title  🏷️  \\n meta ····· time ]', Comp: S8 },
    { title: 'In-body element (progress bar)', slots: '[ title ····· meta \\n ▬▬▬▬▬── ]', Comp: S9 },
    ...
    { title: 'Section headers (grouped)', slots: 'GROUP A\\n  row \\n  row \\n GROUP B ...', Comp: S13 },
    ...
    { title: 'Pinned section (recent + all)', slots: '⏱ RECENT\\n row \\n ─── \\n ALL\\n row ...', Comp: S19 },
