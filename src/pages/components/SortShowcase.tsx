...
function MultiKey() {
  ...
  const toggleDir = (i: number) =>
    setKeys((ks) =>
      ks.map((k, j) =>
        j === i ? { ...k, dir: k.dir === 'asc' ? 'desc' : 'asc' } : k,
      ),
    )
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-xl">
      {opts.map((o) => {
        ...
        return (
          ...
          >
            <o.Icon
              className={`w-4 h-4 ${on ? 'text-zinc-900' : 'text-zinc-500'}`}
            />
...
export default function SortShowcase() {
  return (
    <div className="max-w-5xl space-y-3">
      ...
      <Frame
        label="7. Menu (with check + direction)"
        note="ドロップダウンに項目+順序を両方"
      >
        <MenuWithCheck />
      </Frame>
