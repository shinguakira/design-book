...
function MaskDate() {
  ...
  const onChange = (raw: string) => {
    ...
    let out = d;
    if (d.length > 4)
      out = `${d.slice(0, 4)}/${d.slice(4, 6)}${d.length > 6 ? "/" + d.slice(6) : ""}`;
      ...
      {open && (
        <div className="absolute z-20 mt-1 rounded-md border border-zinc-200 bg-white shadow-lg p-3 w-64">
          <div className="flex items-center justify-between mb-2">
            <button
              onClick={() =>
                setD({ ...d, m: d.m === 0 ? 11 : d.m - 1, y: d.m === 0 ? d.y - 1 : d.y })
              }
            ...
            <button
              onClick={() =>
                setD({ ...d, m: d.m === 11 ? 0 : d.m + 1, y: d.m === 11 ? d.y + 1 : d.y })
              }
              className="p-1 rounded hover:bg-zinc-100"
            >
...
function ClockDial() {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="font-mono text-sm text-zinc-700">{String(h).padStart(2, "0")}:00</div>
    </div>
  );
}
      ...
      <section className="rounded-md border border-blue-200 bg-blue-50 p-4 text-xs text-blue-900">
        ...
        <ul className="list-disc pl-4 space-y-0.5 flex flex-col gap-0">
          <li className="flex items-start gap-1">
            <Calendar className="w-3 h-3 mt-0.5 flex-none" />
            シンプルなら native 系 — スマホは OS ピッカーで最強
          </li>
          ...
          <li className="flex items-start gap-1">
            <Clock className="w-3 h-3 mt-0.5 flex-none" />
            時刻だけ・分刻みが荒い → time slot grid
          </li>
