...
const TOOLS: Tool[] = [
  { name: 'Figma', kind: 'デザイン', desc: 'コラボ前提のデザインツール', users: '4M+', gradient: 'from-pink-400 to-rose-500' },
  { name: 'Sketch', kind: 'デザイン', desc: 'macOS の老舗', users: '1M+', gradient: 'from-amber-400 to-orange-500' },
  { name: 'Adobe XD', kind: 'デザイン', desc: 'Adobe の UI/UX', users: '500K+', gradient: 'from-fuchsia-400 to-purple-500' },
  { name: 'Linear', kind: 'プロジェクト管理', desc: '高速な issue tracker', users: '300K+', gradient: 'from-violet-400 to-indigo-500' },
  { name: 'Notion', kind: 'ナレッジ管理', desc: 'all-in-one workspace', users: '30M+', gradient: 'from-zinc-500 to-zinc-700' },
  { name: 'Slack', kind: 'コミュニケーション', desc: 'チームチャット', users: '20M+', gradient: 'from-violet-500 to-fuchsia-500' },
  { name: 'Discord', kind: 'コミュニケーション', desc: 'コミュニティ', users: '150M+', gradient: 'from-indigo-400 to-violet-500' },
  { name: 'GitHub', kind: '開発', desc: 'Git ホスティング', users: '100M+', gradient: 'from-zinc-700 to-zinc-900' },
  { name: 'VS Code', kind: '開発', desc: 'Microsoft 製エディタ', users: '15M+', gradient: 'from-sky-400 to-blue-600' },
  { name: 'Vercel', kind: '開発', desc: 'FE ホスティング', users: '500K+', gradient: 'from-zinc-800 to-black' },
]
      ...
      {mode === 'list' ? (
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {TOOLS.slice(0, 6).map((t) => (
            <li
              key={t.name}
              className="rounded-md border border-zinc-200 p-3 text-center"
            >
    ...
    <div className="rounded-md border border-zinc-200 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 bg-zinc-50 border-b border-zinc-200 text-xs text-zinc-500">
        <input
          ...
          checked={allChecked}
          onChange={() =>
            setSelected(allChecked ? new Set() : new Set(TOOLS.map((t) => t.name)))
          }
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Frame
          label="Two-column"
          note="CSS columns で2段。テキスト中心のリンク一覧に。"
        >
        ...
        {/* 4 Multi-column 3 */}
        <Frame
          label="Multi-column (3)"
          note="さらに密度を上げる。フッターのリンク群など。"
        >
        ...
        <Frame
          label="Grouped (with headers)"
          note="同種をまとめる。設定・連絡先・受信箱。"
        >
        ...
        <Frame
          label="Avatar list"
          note="顔/イニシャル + 名前 + サブ情報。チームメンバー・受信箱。"
        >
        ...
        <Frame
          label="Switchable view"
          note="ユーザーが List ⇄ Grid を切替える。EC・写真管理。"
        >
          <SwitchableView />
        </Frame>
        ...
        {/* 13 Selectable / bulk */}
        <Frame
          label="Selectable (bulk)"
          note="チェックで複数選択 → 上部に一括アクションバー。"
        >
          <Selectable />
        </Frame>
