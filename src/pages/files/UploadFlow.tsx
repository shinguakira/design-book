...
function LiveDemo() {
        ...
        window.setTimeout(() => {
          ...
          const tick = () => {
            setFiles((prev) => {
              ...
              const next = Math.min(
                100,
                target.progress + 6 + Math.random() * 8,
              )
              if (next >= 100) {
                return prev.map((x) =>
                  x.id === f.id
  ...
  const retryFailed = () => {
    setFiles((prev) =>
      prev?.map((x) =>
        x.state === 'failed' ? { ...x, state: 'queued', progress: 0 } : x,
      ) ?? null,
    )
        ...
        <div>
          <div className="px-4 py-3 border-b border-zinc-200 bg-zinc-50">
            <div className="flex items-center justify-between text-sm">
              <span>
                <b>{done + failed} / {total}</b> ファイル
                {failed > 0 && (
                  <span className="text-rose-600 ml-2">{failed} 件失敗</span>
                )}
              </span>
              <span className="text-xs text-zinc-500 tabular-nums">
                {Math.round(
                  files.reduce((s, f) => s + f.progress, 0) / total,
                )}
                %
              </span>
            </div>
        ...
        >
          ...
          <div
            className={`text-xs mt-1 ${
              dragOver ? 'text-blue-700' : 'text-zinc-500'
            }`}
          >
      ...
      <Frame label="完了 (全成功)">
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 max-w-2xl">
          <div className="px-4 py-3 flex items-center gap-3">
            ...
            <button className="text-sm text-emerald-700 underline">
              共有リンクを取得
            </button>
          </div>
        </div>
      ...
      <Frame label="検証エラー (送信前)" note="サイズ・形式・件数オーバー">
        <div className="space-y-2 max-w-2xl">
          ...
          <div className="flex items-start gap-3 px-4 py-3 rounded-lg border border-rose-200 bg-rose-50 text-sm">
            ...
            <div className="flex-1">
              <div className="font-medium text-rose-900">
                config.exe は許可されていません
              </div>
              <div className="text-xs text-rose-700 mt-0.5">
                許可: PNG, JPG, PDF, ZIP のみ
              </div>
            </div>
            <button className="text-rose-500">
              <X className="w-4 h-4" />
