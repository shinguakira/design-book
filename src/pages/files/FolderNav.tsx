...
function ContextMenu({ ctx, node }: { ctx: TreeCtx; node: TreeNodeData }) {
      ...
      {isFolder && open && node.children && (
        <div className="relative">
          ...
          {node.children.length === 0 ? (
            ...
            node.children.map((c) => (
              <TreeNode key={c.id} node={c} depth={depth + 1} defaultOpen={false} ctx={ctx} />
            ))
          )}
        </div>
      )}
...
function InteractiveTree() {
  ...
  const rename = (id: string, name: string) => {
    setTree((t) => transformTree(t, (n) => (n.id === id ? { ...n, name } : n)));
  };
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <Frame label="ブレッドクラム" note="現在地と階層を1行で">
        <nav className="flex items-center gap-1 text-sm flex-wrap">
          ...
          {["Documents", "Projects", "myapp", "src", "components"].map((seg, i, arr) => (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
          {["Design", "Docs", "2024", "2025", "Archive", "Misc"].map((name, i) => (
            ...
            >
              ...
              />
              <span className="text-xs text-zinc-700 truncate w-full text-center">{name}</span>
            </button>
          ))}
        </div>
      </Frame>

      <Frame label="ピン留め / 最近使った" note="サイドバー上部に置く一覧 (VS Code / Finder)">
      ...
      <Frame label="空フォルダの表現" note="アイコンの違い + 補助テキスト">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="text-center p-6 border border-zinc-200 rounded-lg">
            ...
            <div className="text-xs text-zinc-500 mt-1">アイコンは薄く、塗り無しに</div>
          </div>
          <div className="text-center p-6 border-2 border-dashed border-zinc-300 rounded-lg">
            ...
            <div className="text-xs text-zinc-500 mt-1">枠を破線にしてヒント</div>
          </div>
          <div className="text-center p-6 border border-zinc-200 rounded-lg">
            <FolderSearch className="w-12 h-12 text-zinc-400 mx-auto" />
            <div className="text-sm font-medium mt-2">該当なし</div>
            <div className="text-xs text-zinc-500 mt-1">検索結果ゼロ件はFolderSearch</div>
          </div>
        </div>
      </Frame>
    </div>
