...
const ROWS: Row[] = [
  ...
  },
  {
    ...
    meanings: [
      ...
      {
        region: "中国",
        meaning: "緑の帽子は「妻が浮気している夫」を意味し、強い侮辱",
        tone: "neg",
      },
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <section className="space-y-4">
        {ROWS.map((r) => (
          <div key={r.color} className="rounded-lg border border-zinc-200 bg-white overflow-hidden">
            ...
            <div className="divide-y divide-zinc-100">
              {r.meanings.map((m, i) => (
                <div key={i} className="flex items-start gap-3 px-5 py-2.5 text-sm">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs ${toneClass[m.tone]}`}>
                    {m.meaning}
                  </span>
                </div>
              ))}
