  ...
  return (
    <div className="max-w-4xl space-y-6">
      <section className="text-sm text-zinc-700 leading-relaxed space-y-2">
        <p>
          色とアクセシビリティは表裏一体。「特定の人」のための配慮は、結局すべての人を助ける (carpet
          cut = curb cut effect)。
        </p>
        <p className="text-zinc-600">
          以下はユーザー像ごとの設計上の Do /
          Don't。色だけの話ではなく、色を中心とした包括的配慮として。
        </p>
      </section>
            ...
            <div className="px-5 py-4 space-y-1.5 text-sm">
              {g.tips.map((t, i) => (
                <div key={i} className="flex gap-2">
                  {t.do && (
                    <>
                      <span className="text-emerald-600 font-semibold shrink-0">✓ Do</span>
                      <span className="text-zinc-700">{t.do}</span>
                    </>
                  )}
                  {t.dont && (
                    <>
                      <span className="text-rose-600 font-semibold shrink-0">✗ Don't</span>
      ...
      <section className="rounded-lg border border-blue-200 bg-blue-50 p-5 text-sm text-blue-900">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>Material Design — Accessibility</li>
          <li>Microsoft Inclusive Design Toolkit</li>
        </ul>
      </section>
    </div>
  );
