function Cell({ label, code, anim }: { label: string; code: string; anim: string }) {
  return (
...
const STAGGER_ITEMS = ["寿司", "ピザ", "ハンバーガー", "ラーメン", "サラダ"];

export default function Entrance() {
  return (
    <div className="max-w-5xl space-y-6">
      <p className="text-sm text-zinc-700">
        要素が画面に現れる時のアニメーション。自動で繰り返し再生されます。 本物のUIでは「出現 →
        静止」の1回だけで、戻る動きはありません。
      </p>
      ...
      <section className="rounded-lg border border-blue-200 bg-blue-50 p-5 text-sm text-blue-900">
        ...
        <ul className="list-disc pl-5 space-y-1">
          <li>
            このページのデモは比較のために大きく・繰り返しているが、 本番は{" "}
            <b>60〜120px の動き + 250〜400ms</b> で1回だけ
          </li>
          <li>
            初回マウントのみ。再表示のたびに animate-fade-in を付けると
            ユーザーに「何かが新しい」と誤認させる
          </li>
