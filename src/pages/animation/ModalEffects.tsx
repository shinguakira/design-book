...
const VARIANTS: Variant[] = [
  ...
  },
  {
    ...
    Icon: Scroll,
    label: "Drop",
    description: "巻物が天井から落ちてくる → モーダルが pop → 各セクションが軽くバウンドして着地。",
    phases: [
      "0–720ms: 巻物が上から落ちる",
      "720ms: モーダル pop",
...
export default function ModalEffects() {
  ...
  const trigger = (variant: DrawerAnim) => setOpen({ variant, key: Date.now() });

  return (
    <div className="max-w-5xl space-y-6">
      <p className="text-sm text-zinc-700 leading-relaxed">
        モーダルを「ただ出す」だけでなく、世界観に合わせた演出を入れるパターン。 pubmed-search
        プロジェクトで実装した3種類の「紙」モーダル — 封筒・新聞・巻物
        を移植。クリックで各演出を体験できます。
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {VARIANTS.map((v) => (
          ...
          >
            ...
            <div className="text-xs text-zinc-500 mt-1 leading-relaxed">{v.description}</div>
          </button>
        ))}
        <button
      ...
      <section className="rounded-lg border border-blue-200 bg-blue-50 p-5 text-sm text-blue-900">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>
            注意: 720ms 以上の delay は「操作が遅い」と感じさせるリスク。
            実プロダクトでは初回のみ・スキップ可能にする等を検討
          </li>
        </ul>
      </section>
      ...
      >
        <article className="space-y-5 text-[#2a1d10]">
          <header className="pm-section space-y-2 border-b border-[#b89e6b]/60 pb-3">
            ...
            <p className="text-xs italic text-[#7a5a30]">Demo · {new Date().getFullYear()}</p>
          </header>
          ...
          <section className="pm-section space-y-2">
            ...
            <p className="font-serif text-[14px] leading-relaxed text-[#2a1d10]/90">
              ...
              を backdrop に付けて、その下の <code>.mail-courier</code> / <code>.pm-flap</code> /{" "}
              <code>.pm-section</code> / <code>.pm-knife</code>
              などに CSS keyframes が割り当たります。 各要素は親の data-anim
              を見て自分の動きを決めるので、 variant を切り替えるだけで挙動が変わります。
            </p>
          </section>
