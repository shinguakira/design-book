      ...
      {parts.map((part, i) => {
        ...
        return (
          <span key={i}>
            {inlineParts.map((p, j) =>
              j % 2 === 1 ? (
                <code key={j} className="bg-zinc-100 px-1 py-0.5 rounded text-xs font-mono">
                  {p}
                </code>
              ) : (
  ...
  const streamResponse = (full: string, parentId: number) => {
    ...
    const step = () => {
      ...
      if (i < full.length) {
        ...
        setMessages((m) => [...m, { id: parentId + 1, role: "assistant", text: full }]);
        setStreaming(null);
      }
    };
    setTimeout(step, 350);
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <p className="text-sm text-zinc-700 leading-relaxed">
        ...
        の典型UI。下の入力欄に何か打って送信すると、ストリーミングで応答が流れます。生成中は停止可、各メッセージは再生成・コピー・編集可能。
      </p>
                ...
                <div className="flex gap-3 animate-slide-in-up">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-2">
                      ...
                      >
                        {copiedId === m.id ? (
                          <span className="text-xs text-emerald-600">コピー済</span>
                        ) : (
