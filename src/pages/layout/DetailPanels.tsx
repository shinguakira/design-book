...
import { Mail, Star, X, ChevronLeft, Reply, Archive, Trash2 } from "lucide-react";
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <div className="text-xs text-zinc-400 shrink-0 ml-2">{it.time}</div>
              </div>
              <div
              ...
              <div className="text-xs text-zinc-500 truncate mt-0.5">{it.preview}</div>
            </div>
            {it.starred && <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-400 mt-1" />}
          </button>
          {expanded?.id === it.id && (
            <div className="px-4 pb-4 bg-zinc-50 border-t border-zinc-200 animate-slide-in-down">
              <div className="text-sm text-zinc-700 whitespace-pre-wrap pt-3">{expanded.body}</div>
            </div>
          )}
        </li>
      ))}
...
const PROS_CONS: Record<Mode, { good: string[]; bad: string[] }> = {
  "right-overlay": {
    good: ["メイン画面の文脈を残しつつ詳細を見せられる", "実装が比較的シンプル"],
    bad: ["一覧と詳細を並べて比較できない", "画面が狭いと窮屈"],
  },
  "right-push": {
    good: ["一覧と詳細を同時に見られる (Mail.app / Slack)", "リスト内を次々と確認しやすい"],
  ...
  "bottom-sheet": {
    good: ["モバイルで親指の届く位置に詳細を出せる", "片手操作と相性が良い"],
    bad: ["縦に長いコンテンツは見切れる", "PCで使うと「下から出る」違和感"],
  },
  modal: {
    good: ["集中して見せたい時 (確認・編集)", "モバイル/PC両対応"],
    bad: ["リスト内を次々確認するのには不向き (毎回閉じる必要)", "背景の操作が止まる"],
  },
  "inline-expand": {
    good: ["URLや状態が変わらない、コンテキスト切替なし", "短い詳細・FAQに最適"],
    bad: ["長い詳細はリストが破綻", "上下にジャンプして場所を見失いやすい"],
  },
  "new-page": {
    good: ["詳細をフルに使える、URL共有可能", "戻る/進むのブラウザ操作と一致"],
    bad: ["リストの文脈を完全に失う", "リスト内を巡回するには「戻る」が必要"],
  },
};
    ...
    <div className="max-w-5xl space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
          {MODES.map((m) => (
            ...
            >
              ...
              <div className="text-[11px] text-zinc-500 mt-0.5 font-mono">{m.subtitle}</div>
            </button>
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
          ...
          <DemoArea mode={mode} selected={selected} onSelect={setSelected} onClose={close} />
        </div>
      </section>
    ...
    return (
      <div className="h-[520px] overflow-y-auto">
        <List
          ...
          onSelect={(it) => onSelect(selected?.id === it.id ? (null as unknown as Item) : it)}
